...
The weights are distributed as `ikvmt-ocr-detection` and `ikvmt-ocr-recognition` so that each compressed crate stays below the crates.io upload limit. Each model crate includes its own attribution and licenses. Cargo obtains these crates along with other dependencies; no build script or runtime code downloads models. Both files are embedded with `include_bytes!` into the final ikvmt executable, without changing their contents.

A process loads them lazily on its first OCR request and reuses the engine. Recognition supports the upstream Latin alphabet; it does not promise exact console transcription or infer the selected BIOS item.
