...
- 使用英文提交标题和正文；中文文档以 `.zh.md` 结尾。README、Skill、介质 API、架构、协议与验证说明须保持中英文一致。
...
- 适配器变化时检查 `ikvmt media --help`、CLI/API 一致性和真实设备只读接入/拔出；写入、断线与锁调整须有相关回归，不用真实系统盘做破坏性测试。
- 提交已验证的内容后执行发布，确保包内 Git 提交信息可追溯。

## 发布顺序

先发布模型依赖，再发布主程序；未变化且已发布的模型版本可以继续复用，无需随主程序升级。0.3.0 继续使用两个 0.2.0 模型 crate。主程序使用精确模型版本，更新权重时要同步检查依赖约束和归属说明。
